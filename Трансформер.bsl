...
// Читает строку в указанном формате и возвращает данные 1С (Структура, Массив, примитив).
//
// Поддерживаемые форматы: JSON, TOON, CSV, XML.
// При чтении выполняется автотипизация примитивов:
//   true/false/yes/no → Булево, null/nil/~ → Null,
//   числа → Число, даты ISO → Дата.
//
// Параметры:
//   Строка    - Строка - исходная строка с данными.
//   Формат    - Строка - формат входных данных: "JSON" | "TOON" | "CSV" | "XML".
//                         Регистр не важен. Значение по умолчанию: "JSON".
//   Параметры - Структура, Неопределено - дополнительные настройки чтения:
//       * ПрочитатьВСоответствие - Булево - (JSON) возвращать объекты как Соответствие
//                                            вместо Структуры. По умолчанию Ложь.
//       * Разделитель            - Строка - (CSV, TOON) символ-разделитель полей.
//                                            CSV: ";" по умолчанию. TOON: "," по умолчанию.
//       * ЕстьЗаголовки         - Булево - (CSV) считать первую строку заголовками колонок.
//                                            По умолчанию Истина.
//
// Возвращаемое значение:
//   Произвольный - Структура, Массив, Строка, Число, Булево, Дата или Null
//                  в зависимости от содержимого входной строки.
//
// Пример:
//   Данные = Трансформер.Прочитать(СтрокаJSON, "JSON");
//   Данные = Трансформер.Прочитать(СтрокаXML, "XML");
//
...
// Сериализует данные 1С в строку указанного формата.
//
// Поддерживаемые форматы: JSON, TOON, CSV, XML.
// Принимает Структуру, Соответствие, Массив, ТаблицаЗначений и примитивы.
// ТаблицаЗначений автоматически преобразуется в массив структур перед записью.
// При записи в CSV вложенные структуры автоматически «сплющиваются» (flatten).
//
// Параметры:
//   Данные    - Произвольный - данные для сериализации (Структура, Массив,
//                               ТаблицаЗначений, Соответствие, примитив).
//   Формат    - Строка - формат выходной строки: "JSON" | "TOON" | "CSV" | "XML".
//                         Регистр не важен. Значение по умолчанию: "JSON".
//   Параметры - Структура, Неопределено - дополнительные настройки записи:
//       * Форматировать   - Булево - (JSON, XML) добавлять отступы и переносы строк.
//                                     По умолчанию Ложь.
//       * Разделитель     - Строка - (CSV) символ-разделитель. По умолчанию ";".
//                                    (TOON) символ-разделитель. По умолчанию ",".
//       * СЗаголовками    - Булево - (CSV) записывать первую строку с именами колонок.
//                                     По умолчанию Истина.
//       * КорневойЭлемент - Строка - (XML) имя корневого тега. По умолчанию "root".
//       * ЭлементМассива  - Строка - (XML) имя тега для элементов массива.
//                                     По умолчанию "item".
//
// Возвращаемое значение:
//   Строка - результат сериализации. Пустая строка, если данные пусты.
//
// Пример:
//   JSON = Трансформер.Записать(Данные, "JSON", Новый Структура("Форматировать", Истина));
//   CSV  = Трансформер.Записать(МассивСтруктур, "CSV");
//
...
// Конвертирует строку из одного формата в другой за один вызов.
//
// Выполняет последовательно Прочитать() и Записать() с раздельными параметрами.
// Удобно для прямого преобразования, например XML → JSON или JSON → TOON.
//
// Параметры:
//   Строка            - Строка - исходная строка с данными.
//   ФорматИсточник    - Строка - формат входных данных: "JSON" | "TOON" | "CSV" | "XML".
//   ФорматПриёмник    - Строка - формат выходных данных: "JSON" | "TOON" | "CSV" | "XML".
//   ПараметрыЧтения   - Структура, Неопределено - настройки для Прочитать().
//                        См. описание параметров функции Прочитать().
//   ПараметрыЗаписи   - Структура, Неопределено - настройки для Записать().
//                        См. описание параметров функции Записать().
//
// Возвращаемое значение:
//   Строка - данные, сериализованные в формат приёмника.
//
// Пример:
//   TOON = Трансформер.Конвертировать(СтрокаXML, "XML", "TOON");
//   JSON = Трансформер.Конвертировать(СтрокаCSV, "CSV", "JSON", ,
//              Новый Структура("Форматировать", Истина));
//
...
// Преобразует ТаблицуЗначений в строку формата TOON (табличный массив).
//
// Формирует компактное табличное представление: заголовок с именами колонок
// и строки значений. Удобно для передачи табличных данных в промпт LLM.
// Если таблица пуста, возвращает заголовок с нулевым счётчиком.
//
// Параметры:
//   Таблица      - ТаблицаЗначений - исходная таблица.
//   ИмяМассива   - Строка - имя массива в заголовке TOON. По умолчанию "data".
//   Разделитель  - Строка - символ-разделитель значений. По умолчанию ",".
//
// Возвращаемое значение:
//   Строка - данные в формате TOON табличного массива.
//
// Пример:
//   Таблица = Запрос.Выполнить().Выгрузить();
//   TOON = Трансформер.ТаблицаВТООН(Таблица, "items", ",");
//
...
// Преобразует строку TOON (табличный массив) в ТаблицуЗначений.
//
// Парсит TOON-строку и ищет первый табличный массив (или массив объектов).
// Колонки таблицы создаются по ключам первого элемента найденного массива.
// Если подходящий массив не найден, возвращает пустую ТаблицуЗначений.
//
// Параметры:
//   СтрокаTOON  - Строка - данные в формате TOON.
//   Разделитель - Строка - символ-разделитель, использованный при записи.
//                           По умолчанию ",".
//
// Возвращаемое значение:
//   ТаблицаЗначений - таблица с данными из TOON.
//
// Пример:
...
// Преобразует ТаблицуЗначений в строку CSV.
//
// Обёртка над Записать(, "CSV") с удобными позиционными параметрами.
//
// Параметры:
//   Таблица      - ТаблицаЗначений - исходная таблица.
//   Разделитель  - Строка - символ-разделитель. По умолчанию ";".
//   СЗаголовками - Булево - добавлять строку с именами колонок. По умолчанию Истина.
//
// Возвращаемое значение:
//   Строка - данные в формате CSV.
//
// Пример:
...
// Преобразует строку CSV в ТаблицуЗначений.
//
// Парсит CSV, создаёт ТаблицуЗначений. Если ЕстьЗаголовки = Истина,
// первая строка используется как имена колонок. Значения в ячейках
// проходят автотипизацию (числа, даты, булево).
//
// Параметры:
//   СтрокаCSV     - Строка - данные в формате CSV.
//   Разделитель   - Строка - символ-разделитель. По умолчанию ";".
//   ЕстьЗаголовки - Булево - первая строка содержит имена колонок.
//                             По умолчанию Истина.
//
// Возвращаемое значение:
//   ТаблицаЗначений - таблица с данными из CSV.
//
// Пример:
//   Таблица = Трансформер.CSVВТаблицу(ТекстCSV, ";", Истина);
//
...
// ИСПРАВЛЕНО v1.0.5: добавлена обработка Структуры/Соответствия и сплющивание вложенных объектов
Функция ЗаписатьФорматCSV(Знач Данные, Знач Параметры)
    ...
    // v1.0.5: Структура/Соответствие → извлечь вложенный массив или обернуть
    ...
    // v1.0.5: сплющить вложенные структуры в элементах массива
...
// v1.0.5: Рекурсивное сплющивание вложенных структур для CSV
